...
```{r}
names <- c('John', 'James', 'Sandy', 'Eric', 'Cameron')
names
```
...
```{r}
#similar to numpy and pandas, but this is default in R
...
student_classgr <- c(65, 82, 91)

#Create a dataframe from these 3 vectors such that each vector is its own column
df <- data.frame(student_name, student_grade_level, student_classgr)
df
```
...
```{r}
#harder than adding new columns
#easier to do this in tibbles

student_name <- c('Jan','Jimmy')
student_grade_level <- c('Freshman', 'Junior')
student_classgr <- c(95,67)
student_subject <- c('history', 'English')
...
my_matrix <- matrix(
  ...
  ncol <- 5,
  byrow <- FALSE,
  dimnames <- list(c("Row 1", "Row 2"), c("COL 1", "COL 2", "COL 3", "My Grades", "COL5"))
)
...
```{r}
m_1 <- matrix(c(1,2,3,4,5,6), nrow <- 3)
m_2 <- matrix(c(4,5,6,7,8,9), nrow <- 3)
...
my_factor <- factor(c('Blue', 'Green', 'Brown', 'Blue', 'Hazel', 'Black', 'Green'))
my_factor
```
...
```{r}
 my_factor <- factor(c('Blue', 'Green', 'Brown', 'Blue', 'Hazel', 'Black', 'Green'),
                     levels <- c('Blue', 'Green', 'Hazel', 'Black'))
 #levels helps create custom order of sorting values 
 sort(my_factor)
```
...
```{r}
first_vec <- c(1,2,3,4,5,6,7,8,9)
second_vec <- c(9,8,7,6,5,4,3,2,1)

combined <- c(first_vec, second_vec)
combined

vec_array <- array(
  combined,
  dim <- c(3,3,2),
  dimnames <- list(c('Row 1', 'Row 2', 'Row 3'), c('Col 1', 'Col 2', 'Col 3'), c('Matrix Uno', 'Matrix Dos'))
)
vec_array
```
